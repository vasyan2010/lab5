/* если одна и та же дуга вводится 2 раза, то ранние данные просто игнорируются */
#include  <stdio.h>
#include  <windows.h>
#include  <conio.h>
#include  <stdlib.h>
...
#include  <iostream>
using namespace std;
const int NONE=1000*10000; //оч большое число // будет время - можно понатыкать if чтобы сделать NONE = -1 и не пориться
...
class GR{   
	private:
		struct node  //список городов
		{
			node *next; 	//ссыль на след элемент списка 
			char name[maxSizeNodeName];		//можно было запилить динамический массив на сколько угодно символов, но мне лень
			unsigned int nameSize; // омск - 4 (без \0, хотя он и присутствует в конце)
		};
		
		node* nodeHead;				//указатель на начало списка наименований узлов
		int * matrixH;				//массив высот
		int * matrixE;
		int ** matrixC;				//матрица С
		int ** matrixF;				//матрица F
		unsigned int nodeListSize;	//кол-во узлов в графе (нужно для задания размеров матриц
		unsigned int matrixSize;	//кол-во выделенного места под матрицу (для изменения размеров матриц)
	public:		
		GR()			//конструктор 
		{ 
			nodeHead=NULL;	
		...
		bool parser(char * str) //считывание строки
		{
			...
			if (!errorInput)
			{
				memcpy(node1Name, str+nowPoint, node1NameSize); //для поиска
				node1Name[node1NameSize]='\0';
				searchResult1=search(node1Name,node1NameSize);
				if (searchResult1==NONE) //такого не найдено
				{
					addNewnode(node1Name, node1NameSize);
					searchResult1=nodeListSize-1;
				}	
				nowPoint+=node1NameSize+1; //+1 чтобы не считать " " в str в след раз/
				...
				if (!errorInput)
				{
					memcpy(node2Name, str+nowPoint, node2NameSize); //для поиска
					node2Name[node2NameSize]='\0';
					searchResult2=search(node2Name,node2NameSize);
					if (searchResult2==NONE) //такого не найдено 
					{
						addNewnode(node2Name, node2NameSize);
						searchResult2=nodeListSize-1;
					}		
					nowPoint+=node2NameSize+1; //+1 чтобы не считать " " в str в след раз
					
					/*пропускная способность дуги из узла 1 в узел 2*/
					unsigned int costSize;
					char cost[12]; //тут 11 потому что максимальное число это 2 147 483 647 + один чар для терминатора+ 1 для минуса(в случае некоректноого ввода
					...
					{	
						...
						if (atoi(cost)<=0) //atoi возвращает 0 в случае некорректного ввода
							errorInput=true;
		...
		void printMatrix()
		{
			...
			if (matrixE!=NULL)
			{
				...
				cout<<"matrixE = NULL\n";
		cout<<endl;		
		//	cout<<"nodeListSize "<<nodeListSize<<endl;
		//	cout<<"matrixSize "<<matrixSize<<endl;
		}
		int search(char * nodeName, unsigned int nameSize) //поиск по node (nameSize включает \0) омск - nameSize = 4
		{
			...
			return ((!found)?NONE:numInList); //ВОЗВРАЩАЕТ НОМЕР В МАТРИЦЕ (c нуля)
		}
		void changeMatrix()
		{
			unsigned int source;
			...
			//тут нужно обрезать матрицу, потому что потом надо либо флаги добалсять, либо будет утечка памятьи
			...
			source=search(&S,1);
			if (source!=0) 	//исток не на нулевом месте
			{
				//ставим на первое месото S (в конец списка node)
				
				node * nodeS=nodeHead;
				for (unsigned int i = 0; i < nodeListSize-source-1; i++) 	//поиск nodeS    	
	    			nodeS=nodeS->next;	
	    		cout<<nodeS->name<<endl;
	    		
	    		node * tmp=nodeHead; //элемент перед S в списке
	    		if (tmp!=nodeS)
	    			while (tmp->next!=nodeS)
	    				tmp=tmp->next;
	    		tmp->next=nodeS->next;	
	    		nodeS->next=NULL;
	    		while (tmp->next!=NULL) // S точно не в конце списка, поэтому 1 раз прога в цикл зайдет
	    			tmp=tmp->next; //доходим до конца
	    		tmp->next=nodeS;
	    		
	    		
	    		
				int **newMatrixC =  new int*[nodeListSize]; 	//создаем новую матрицу С
	    		for (unsigned int i = 0; i < nodeListSize; i++) 	    	
	    			newMatrixC[i] = new int[nodeListSize];			    
			   	/* дальше страшно много if, но нужно рассмотреть 9 частей матрицы, по другому не получается */
			   	//тут можно вложить if друг в друга чтобы выиграть в скорости, но тогда код получится еще страшнее
			    for (unsigned int i = 0; i < nodeListSize; i++)  //вносим данные в новую матрицу
	    			for(unsigned int j = 0; j < nodeListSize; j++)
	    			{
	    				if ((i==source)&&(j==source))		//тут можно ничего не прописывать, т.к. в любом случае будет 0
			...
			source=search(&T,1);
			if (source!=nodeListSize-1) 	//сток не на последнем месте
			{
				int **newMatrixC =  new int*[nodeListSize]; 	//создаем новую матрицу С
	    		for (unsigned int i = 0; i < nodeListSize; i++) 	    	
	    			newMatrixC[i] = new int[nodeListSize];			    
			   	/* дальше страшно много if, но нужно рассмотреть 9 частей матрицы, по другому не получается */
			   	//тут можно вложить if друг в друга чтобы выиграть в скорости, но тогда код получиться еще страшнее
			    for (unsigned int i = 0; i < nodeListSize; i++)  //вносим данные в новую матрицу
	    			for(unsigned int j = 0; j < nodeListSize; j++)
	    			{
	    			//	cout<<"["<<i<<"] ["<<j<<"]";
		...
		void addNewnode(char * nodeName, unsigned int nodeNameSize)
		{
			...
			nodeListSize++; 
			if (nodeListSize>matrixSize) //изменяем размер матрицы если надо
			{
			    ...
			    for (unsigned int i = 0; i < newMatrixSize; i++)  //memcpy(newMatrix, matrix, sizeof(int) * matrixSize*matrixSize);
...
int main()
{
	...
	const unsigned int strMaxSize=128; //максимальная длина строки в файле
	char str[strMaxSize]; //строка, в которую считываем	
	GR sewerage; //канализационная система
	
	//построчно считываем файл и заносим данные в матрицу С
    ...
    if (!errorReading)
    {
	    ...
	    sewerage.changeMatrix(); //перенос С в левый верхний угол и T в правый нижний
